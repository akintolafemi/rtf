...
<html>
  ...
  <head>
    ...
    <title>Royal Theatre Family - <?php echo 'Videos by '.$rtfname;?></title>
  ...
  <body class="theme-style-1">
    ...
    <section class="cp-category cp-photo-box">
      <ul>
        <?php
          if (!isset($_SESSION['member'])) $sql = "SELECT * FROM `tb_videos_uploaded` WHERE `col_member` = '".mysqli_real_escape_string($db, $rtfname)."' AND `col_share` != 'membersalone' ORDER BY `col_id` DESC";
          else { $sql = "SELECT * FROM `tb_videos_uploaded` WHERE `col_member` = '".mysqli_real_escape_string($db, $rtfname)."' ORDER BY `col_id` DESC"; }
          videoToShoot('members/video.media', $sql);
        ?>
      </ul>
